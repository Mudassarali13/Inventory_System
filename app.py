    ...
    if action == "Return":
        ...
        if not stock.empty and not assign_df[mask].empty:
            ...
            st.markdown(f"""
                ...
                <b>Status:</b> {assignment['status']}<br>
                <b>Assigned Quantity:</b> {assignment.get('assigned_quantity', 1)}
                </div>
            """, unsafe_allow_html=True)
            img_col, bar_col = st.columns(2)
            img_path = product.get('image', '')
            ...
            if st.button("Return Product"):
                ...
                # Add back the assigned quantity (default to 1 if not present)
                qty_to_return = int(assignment.get('assigned_quantity', 1))
                df.loc[df['id_stripped'] == product_id_str, 'quantity'] = int(df[df['id_stripped'] == product_id_str]['quantity'].values[0]) + qty_to_return
        ...
        # New: Input for quantity to assign
        current_qty = int(stock[stock['id'] == selected_id]['quantity'].values[0])
        assign_qty = st.number_input("Quantity to Assign", min_value=1, max_value=current_qty, value=1)
        if st.button("Assign Product"):
            if current_qty < assign_qty:
                st.warning(f"Only {current_qty} units available. Cannot assign {assign_qty}.")
                return
            if assign_qty <= 0:
                st.warning("Please enter a valid quantity to assign.")
                return
            df.loc[df['id'] == selected_id, 'quantity'] = current_qty - assign_qty
            new_assignment = {
                "user": user,
                "role": role,
                ...
                "return_date": "",
                "assigned_quantity": assign_qty  # New field
            }
            assign_df = pd.concat([assign_df, pd.DataFrame([new_assignment])], ignore_index=True)
            save_data(df)
            save_assignments(assign_df)
            st.success(f"{assign_qty} unit(s) of {stock['name'].values[0]} assigned to {user}.")
